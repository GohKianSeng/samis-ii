...
---$ Alter Procedure dbo.usp_addNewMember 
IF OBJECT_ID(N'dbo.usp_addNewMember') IS NULL
BEGIN
    PRINT 'Create procedure : dbo.usp_addNewMember'
    EXECUTE('CREATE PROCEDURE dbo.usp_addNewMember AS RETURN 0') 
END
GO

PRINT 'Alter procedure : dbo.usp_addNewMember'
GO
...
---$ Alter Procedure dbo.usp_getListOfTempMembersForApproval 
IF OBJECT_ID(N'dbo.usp_getListOfTempMembersForApproval') IS NULL
BEGIN
    PRINT 'Create procedure : dbo.usp_getListOfTempMembersForApproval'
    EXECUTE('CREATE PROCEDURE dbo.usp_getListOfTempMembersForApproval AS RETURN 0') 
END
GO

PRINT 'Alter procedure : dbo.usp_getListOfTempMembersForApproval'
GO
...
---$ Alter Procedure dbo.usp_UpdateNewMember 
IF OBJECT_ID(N'dbo.usp_UpdateNewMember') IS NULL
BEGIN
    PRINT 'Create procedure : dbo.usp_UpdateNewMember'
    EXECUTE('CREATE PROCEDURE dbo.usp_UpdateNewMember AS RETURN 0') 
END
GO

PRINT 'Alter procedure : dbo.usp_UpdateNewMember'
GO
...
GO
  ...
  SELECT 'GreenFormFullCheck', 'Off'
  
  INSERT INTO dbo.tb_AppModFunc (AppModFuncID, AppModFuncName, URL, Sequence)
  SELECT '1.5.5', 'Web.config', 'settings/samis2webconfig', '5'
  INSERT INTO dbo.tb_AppModFunc (AppModFuncID, AppModFuncName, URL, Sequence)
  SELECT '1.5.6', 'View Sessions Variable', 'settings/samis2sessions', '6'
...
INSERT INTO dbo.tb_Roles(RoleID, RoleName)
SELECT -1, 'Public'
...
--SELECT (select TOP 100 ISNULL(B.ICPhoto,'') AS ICPhoto, ISNULL(Name,B.EnglishName) AS Name, ISNULL(A.NRIC, B.NRIC) AS NRIC, ISNULL(ISNULL(A.Email,B.Email),' ') AS Email from dbo.tb_Users AS A
