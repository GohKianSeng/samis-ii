<%@ Page Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage" %>
<%@ Import Namespace="DOS.Models" %>
<%@ Import Namespace="System.Xml.Linq" %>
<asp:Content ID="aboutTitle" ContentPlaceHolderID="TitleContent" runat="server">
    Search By Marital Status
</asp:Content>

<asp:Content ID="aboutContent" ContentPlaceHolderID="MainContent" runat="server">
<script type="text/javascript" src="/Content/scripts/jquery-1.6.4.min.js"></script>
<link rel="stylesheet" type="text/css" href="/Content/css/TabsView.css">

<%if (HttpContext.Current.IsDebuggingEnabled){%>
    <script type="text/javascript" src="/Content/scripts/common_function.debug.js"></script>
<%}else{%>
    <script type="text/javascript" src="/Content/scripts/common_function.min.js"></script>
<%}%>

<script language="C#" runat="server">
    void loadmaritalstatus(Object Sender, EventArgs e)
    {
        List<usp_getAllMaritalStatusResult> res = (List<usp_getAllMaritalStatusResult>)ViewData["maritalstatuslist"];
        ListItem item = new ListItem("", "");
        marital_status.Items.Add(item);
        for (int x = 0; x < res.Count; x++)
        {
            item = new ListItem(res.ElementAt(x).MaritalStatusName, res.ElementAt(x).MaritalStatusID.ToString());
            marital_status.Items.Add(item);
        }
    }
</script>

<script type="text/javascript">
    function getMaritalStatusID() {
        return "<%= marital_status.ClientID%>";
    }

    function reloadList() {
        $("#MemberlistIframe").get(0).contentWindow.reloadCase($("#" + getMaritalStatusID()).val());
    }
</script>    

<form id="form1" runat="server">
    <div class="container">
        <ul class="tabs">
            <li><a href="#tab1">Marital Status</a></li>
        </ul>
        <div class="tab_container">
            <div id="tab1" class="tab_content">
                <table class="dottedview" cellspacing="0">
                    <tr>
                        <td style="width: 160px">
                            Marital Status<br>
                            <asp:DropDownList  style=" width:130px" onchange="reloadList()" class="element select medium" OnLoad="loadmaritalstatus" name="marital_status" ID="marital_status" runat="server">
                            </asp:DropDownList>                            
                        </td>
                    </tr>
                </table>
                <br>
                <iframe id="MemberlistIframe" frameborder="0" src="/Reporting.mvc/marriagelist" style="width: 100%; height: 450px;">
                    <p>
                        Your browser does not support iframes.</p></iframe>
            </div>
        </div>
        <p>
            &nbsp;</p>
        &nbsp;
    </div>
    </form>





</asp:Content>